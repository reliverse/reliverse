...
function formatPluginCommandSuggestion(programName: string, path: readonly string[]): string {
  return `${programName} ${path.join(" ")}`;
}

function normalizePluginNameCandidate(value: string): string {
  const withoutScope = value.startsWith("@") ? (value.split("/")[1] ?? value) : value;
  return withoutScope
    .replace(/-rse-plugin$/, "")
    .replace(/-plugin$/, "")
    .toLowerCase();
...
function getPluginNameCandidates(plugin: RemptsPlugin): readonly string[] {
  return [...new Set([plugin.name.toLowerCase(), normalizePluginNameCandidate(plugin.name)])];
}
...
function createPluginNameUnknownCommandHint(options: {
  ...
  const remaining = options.argv.slice(1).filter((segment) => segment !== "--");
  const commandPaths = [
    ...new Set(
  ...
  const matchingPaths =
    remaining.length > 0
      ? commandPaths.filter((path) => path.join(" ") === remaining.slice(0, path.length).join(" "))
      : [];
  const suggestions = (matchingPaths.length > 0 ? matchingPaths : commandPaths)
    .slice(0, 5)
  ...
  if (suggestions.length === 0) return `${prefix} Run --help to discover available commands.`;
        ...
        output.problem({
          ...toStructuredRemptsError(
            new RemptsUsageError(`Unknown command "${discovered.unknownSegment}".`, 1, {
              ...
              hint: pluginNameHint ?? "Run --help to discover available commands.",
              usage: pluginNameHint ? undefined : launcherHelp.usage[0],
            }),
          ),
        });
