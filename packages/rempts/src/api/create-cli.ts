...
export interface CreateCLIOptions {
    ...
    readonly onExit?:
        | ((result: CLIExecutionResult) => Promise<void> | void)
        | undefined;
...
const APPLY_OPTION_DEFINITION: CommandOptionDefinition = {
    type: "boolean",
    description:
        "Execute side effects. Default is preview-only for commands that require apply.",
    inputSources: ["flag"],
};
...
function withSafetyApplyOption(
    ...
    const requiresApply =
        definition.safety?.requiresApply === true ||
        definition.conventions?.supportsApply === true;
...
function toCommandRuntimeInfo<TOptions extends CommandOptionsRecord>(
    ...
    commandNode: Pick<
        CommandNode,
        "directoryPath" | "filePath" | "sourceId" | "sourceKind"
    >,
...
function formatPluginCommandSuggestion(
    programName: string,
    path: readonly string[],
): string {
    return `${programName} ${path.join(" ")}`;
}

function normalizePluginNameCandidate(value: string): string {
...
function getPluginNameCandidates(plugin: RemptsPlugin): readonly string[] {
    return [
        ...new Set([
            plugin.name.toLowerCase(),
            normalizePluginNameCandidate(plugin.name),
        ]),
    ];
}

function createPluginNameUnknownCommandHint(options: {
    readonly cliName: string;
    readonly commandDiagnostics: CommandTreeReport;
    readonly matchedPath: readonly string[];
    readonly plugins: readonly RemptsPlugin[];
    readonly unknownSegment: string;
    readonly argv: readonly string[];
}): string | undefined {
    if (options.matchedPath.length > 0) return undefined;

    const normalizedUnknown = options.unknownSegment.toLowerCase();
    const plugin = options.plugins.find((candidate) =>
        getPluginNameCandidates(candidate).includes(normalizedUnknown),
    );
    if (!plugin) return undefined;

    const remaining = options.argv
        .slice(1)
        .filter((segment) => segment !== "--");
    const commandPaths = [
        ...new Set(
            options.commandDiagnostics.nodes
                .filter(
                    (node: CommandTreeNodeDiagnostic) =>
                        node.chosenCommand?.sourceKind === "plugin" &&
                        node.chosenCommand.sourceId === plugin.name &&
                        node.path.length > 0,
                )
                .map((node: CommandTreeNodeDiagnostic) => node.path.join(" ")),
        ),
    ]
    ...
    const matchingPaths =
        remaining.length > 0
            ? commandPaths.filter(
                  (path) =>
                      path.join(" ") ===
                      remaining.slice(0, path.length).join(" "),
              )
            : [];
    const suggestions = (
        matchingPaths.length > 0 ? matchingPaths : commandPaths
    )
        .slice(0, 5)
        .map((path) => formatPluginCommandSuggestion(options.cliName, path));

    const prefix = `Plugin "${plugin.name}" is loaded, but "${options.unknownSegment}" is not a command namespace.`;
    if (suggestions.length === 0)
        return `${prefix} Run --help to discover available commands.`;

    return `${prefix} Maybe you meant: ${suggestions.join(", ")}?`;
}
...
export async function createCLI(
    ...
    const renderJsonHelp = shouldRenderJsonHelp(
        options.help?.format,
        outputMode,
    );
    ...
    try {
        ...
        const globalFlagDefinitions = getGlobalFlagDefinitions(
            options.globalFlags,
        );
        ...
        const sources = [
            ...
            ...effectivePlugins.map((plugin) =>
                createPluginCommandSource(plugin),
            ),
        ];
        const commandDiagnostics = await inspectCommandTree(sources);
        const discovered = await discoverCommandPath(
            sources,
            parsedGlobals.argv,
        );
        ...
        if (!discovered.commandNode?.loadCommand) {
            const emptyCliHelpText =
                ...
                discovered.availableSubcommands.length === 0
            ...
            if (discovered.unknownSegment) {
                const pluginNameHint = createPluginNameUnknownCommandHint({
                    argv: parsedGlobals.argv,
                    cliName,
                    commandDiagnostics,
                    matchedPath: discovered.matchedPath,
                    plugins: effectivePlugins,
                    unknownSegment: discovered.unknownSegment,
                });
                output.problem({
                    ...toStructuredRemptsError(
                        new RemptsUsageError(
                            `Unknown command "${discovered.unknownSegment}".`,
                            1,
                            {
                                code: "REMPTS_UNKNOWN_COMMAND",
                                hint:
                                    pluginNameHint ??
                                    "Run --help to discover available commands.",
                                usage: pluginNameHint
                                    ? undefined
                                    : launcherHelp.usage[0],
                            },
                        ),
                    ),
                });

                if (!renderJsonHelp && !pluginNameHint) {
                    writeHelp(
                        stderr,
                        renderHelpDocument(launcherHelp, helpColors),
                    );
                }
        ...
        if (!loadCommand) {
            throw new RemptsUsageError(
                "Resolved command node is missing a command loader.",
                1,
            );
        }
        ...
        const owningPlugin =
            resolvedCommandNode.sourceKind === "plugin"
                ? effectivePlugins.find(
                      (plugin) => plugin.name === resolvedCommandNode.sourceId,
                  )
                : undefined;
        ...
        const effectiveCommandOptions = withSafetyApplyOption(
            mergedCommandOptions,
            command,
        );
        ...
        try {
            parsed = await parseArgvTail(
                discovered.remainingArgv,
                effectiveCommand.options,
                env,
            );
        } catch (error) {
            if (
