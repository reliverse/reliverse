---
name: db-conventions
description: Execute the proper Drizzle Kit workflow for database schema migrations in the current monorepo project. Use this when the user asks to create a database migration, update the database schema, or when schema changes need to be applied to the database.
allowed-tools: [Bash, Read, Grep, Edit]
---

# Drizzle Kit Migration Workflow

This skill guides you through the proper database migration workflow using Drizzle Kit for the current monorepo project.
...
- Database schema files in `db/schema/` have been modified
...
After making changes to TypeScript schema files in `packages/db/schema/`, generate a migration:

```bash
cd ${WORKSPACE_DIR}/packages/db
bun run db:generate
```

**What this does:**

- Reads all schema files from `db/schema/`
- Compares with previous migration state
- Generates SQL migration file in `db/migrations/`
...
// packages/db/schema/teams.schema.ts
...
// 5. Commit changes
// git add db/schema/teams.schema.ts
// git add db/migrations/0004_amazing_hulk.sql
...
- `packages/db/schema/` - All schema definitions
- `packages/db/migrations/` - Generated SQL migrations
- `packages/db/migrations/meta/_journal.json` - Migration tracking
