...
export function formatGtbSummary(run: GtbRunResult, colors: GtbSummaryColors): string {
  const lines: string[] = [];

  lines.push("");
  lines.push(colors.heading(run.apply ? "gtb packed tarballs" : "gtb preview"));
  lines.push("");
  ...
  lines.push(`${colors.key("package")} ${colors.value(run.resolvedRoot.name)}`);

  if (run.inputPackageName !== run.packageName) {
    lines.push(`${colors.key("input package")} ${colors.value(run.inputPackageName)}`);
  }
  ...
  lines.push(`${colors.key("platform")} ${colors.value(platformLabel(run.os, run.arch))}`);
  lines.push(`${colors.key("output")} ${colors.value(run.outputDir)}`);
  lines.push(`${colors.key("optional mode")} ${colors.value(run.optionalMode)}`);
  lines.push("");

  lines.push(colors.heading("tarballs"));

  for (const item of run.plan) {
    const badge = item.kind === "root" ? colors.info("root") : colors.info("optional");
    const platform =
      item.kind === "optional"
  ...
  if (run.skipped.length > 0) {
    ...
    for (const skipped of run.skipped) {
      lines.push(`- ${colors.muted(skipped.name)} ${colors.muted(skipped.reason)}`);
    }
  }
  ...
  if (!run.apply) {
    ...
    lines.push(colors.muted("No tarballs were written. Re-run with --apply to execute npm pack."));
    lines.push("");
  ...
  if (run.apply) {
    lines.push("");
    lines.push(
      colors.success(`written ${run.packed.length} tarball${run.packed.length === 1 ? "" : "s"}`),
    );
