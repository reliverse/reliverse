...
export async function buildGtbPlan(options: GtbOptions): Promise<{
    ...
    const requestedSpec = packageSpec(
        options.packageName,
        options.version ?? options.tag,
    );
    const resolvedRoot = await readNpmPackageInfo(
        options.npmBin,
        requestedSpec,
    );
    ...
    const plan: GtbPackagePlanItem[] = [
        {
            ...
            outputFilename: npmTarballFilename(
                resolvedRoot.name,
                resolvedRoot.version,
            ),
            outputPath: npmTarballPath(
                options.outputDir,
                resolvedRoot.name,
                resolvedRoot.version,
            ),
    ...
    for (const [dependencyName, dependencyRange] of optionalEntries) {
        const matchedPlatform = isPlatformOptionalDependency(
            dependencyName,
            options.os,
            options.arch,
        );
...
async function resolveOptionalDependencyInfo(input: {
    ...
    return readNpmPackageInfo(
        input.npmBin,
        packageSpec(input.dependencyName, input.dependencyRange),
    );
}
