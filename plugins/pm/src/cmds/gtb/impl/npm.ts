...
export async function readNpmPackageInfo(npmBin: string, spec: string): Promise<GtbNpmPackageInfo> {
  ...
  if (!isRecord(info)) {
    throw new Error(`npm view did not return package metadata for ${spec}.`);
  }

  const name = readStringProperty(info, "name");
  const version = readStringProperty(info, "version");
  const optionalDependencies = readStringRecordProperty(info, "optionalDependencies");
...
function parseNpmJson(stdout: string): unknown {
  ...
  try {
    ...
    throw new Error(`Failed to parse npm JSON output: ${message}\n\n${trimmed}`);
  }
}
